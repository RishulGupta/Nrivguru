...
function ResultsOverlay({ score, prevScore, jointScores, phase, onRetry, onNext, isLastChunk }: {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/95 flex flex-col p-6 gap-5 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between pt-10 px-2">
        <div>
          ...
          <div className={`text-6xl font-bold tabular-nums mt-1 ${great ? 'text-green-400' : ok ? 'text-violet-400' : 'text-white'}`}>
            {pct}<span className="text-3xl font-normal text-white/30">%</span>
          </div>
          {delta !== null && (
            <p className={`text-sm font-semibold mt-1 ${delta > 0 ? 'text-green-400' : delta < 0 ? 'text-red-400' : 'text-white/30'}`}>
              {delta > 0 ? `+${delta}` : delta} from last try
            </p>
          )}
        </div>
        <div className="text-6xl">{great ? '🔥' : ok ? '💪' : '🌱'}</div>
      </div>

      {/* Fix tip */}
      <div className="rounded-2xl bg-violet-500/8 border border-violet-500/20 p-5 flex-1 flex flex-col justify-center">
        {!great && worstArmJoint ? (
          <>
            <p className="text-violet-300/50 text-[10px] uppercase tracking-widest mb-2">Fix this</p>
          ...
          <>
            <p className="text-green-400/60 text-[10px] uppercase tracking-widest mb-2">Looking great</p>
            <p className="text-white font-semibold text-lg">Arms are on point 🎯</p>
          </>
        )}
        {score.weakerSide && (
          <p className="text-amber-300/60 text-sm mt-3">{score.weakerSide} side needs more focus</p>
        )}
      </div>

      {/* Actions */}
      <div className="flex flex-col gap-3 pb-4">
        <button onClick={onRetry} className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-4 rounded-2xl transition-all text-base">
          🔄 Try again
        </button>
        <button onClick={onNext} className="w-full bg-primary hover:bg-primary/90 text-white font-bold py-4 rounded-2xl transition-all shadow-[0_0_15px_rgba(147,51,234,0.3)] text-base">
